...
/// StoreKit 2 — Proofd Pro is a one-time non-consumable buyout (no subscription).
@MainActor
final class ProStore: ObservableObject {
    static let productID = "com.duolaameng.proofd.pro"
    @Published var product: Product?
    @Published var isPro = false
    @Published var purchasing = false
    var priceText: String { product?.displayPrice ?? "$19.99" }
